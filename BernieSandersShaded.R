...
# hc_coordinates_B<- cbind(HillaryTotals$lon,HillaryTotals$lat)
...
#p1 <-
ggplot(US_states) + geom_map(aes(map_id = region), map = US_states, fill = "grey90", color = "grey50", size = 0.25)+ expand_limits(x = US_states$long, y = US_states$lat) + scale_x_continuous("Longitude") + scale_y_continuous("Latitude") + theme_minimal() + geom_point(data = BernieTotals, aes(x = lon, y = lat), size = 1.5, alpha = 1/5, color = "orange")
